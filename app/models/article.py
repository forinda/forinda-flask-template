...
class Article(BaseModel):
    ...
    @classmethod
    def find_by_slug(cls, slug: str, session=None) -> dict | None:
        """
    ...
    def find_by_category(cls, category_id: str, skip: int = 0, limit: int = 10, session=None) -> list[dict]:
        ...
        return cls.find_many(
            {'category_id': category_id}, skip=skip, limit=limit, sort=[('created_at', -1)], session=session
        )
    ...
    def search(cls, search_term: str, skip: int = 0, limit: int = 10, session=None) -> list[dict]:
        ...
        """
        return cls.find_many(
            {'$text': {'$search': search_term}}, skip=skip, limit=limit, sort=[('created_at', -1)], session=session
        )
